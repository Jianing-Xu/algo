...
/**
 * 二分查找算法集合 包含各种二分查找的变体实现
 */
public class BinarySearch {

  /**
   * 标准二分查找 在有序数组中查找目标值 时间复杂度：O(log n)，空间复杂度：O(1)
   *
   * @param arr    有序数组
   * @param target 目标值
  ...
  /**
   * 查找最后一个等于目标值的元素位置 适用于有重复元素的有序数组
   *
   * @param arr    有序数组（可能有重复元素）
   * @param target 目标值
  ...
  /**
   * 在旋转排序数组中查找目标值 数组原本是升序排列，但在某个点进行了旋转
   *
   * @param arr    旋转排序数组
   * @param target 目标值
   * @return 目标值的索引，如果不存在返回-1
  ...
  /**
   * 在二维矩阵中进行二分查找 矩阵每行从左到右递增，每列从上到下递增
   *
   * @param matrix 二维矩阵
   * @param target 目标值
   * @return 是否找到目标值
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "  第一个 >= " + target + " 的位置: " + findFirstGreaterOrEqual(arr3, target));
