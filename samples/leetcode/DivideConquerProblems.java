...
public class DivideConquerProblems {
     ...
     * 2. 递归求解左右两部分的多数元素
     * 3. 如果左右多数元素相同，返回该元素
     * 4. 否则统计两个候选元素在整个区间的出现次数，返回出现次数多的
     * 
     * @param nums 整数数组
     * @return 多数元素
     */
     ...
     * 快速选择算法（分治思想）：
     * 1. 选择一个基准元素进行分区
     * 2. 如果基准位置正好是第k大的位置，返回该元素
     * 3. 如果基准位置大于k，在左半部分继续查找
     * 4. 如果基准位置小于k，在右半部分继续查找
     * 
     * @param nums 整数数组
     * @param k 第k大
     * @return 第k个最大元素
    ...
    public static void main(String[] args) {
        ...
        System.out.println("单个非空链表: " + mergeKLists(new ListNode[]{createLinkedList(new int[]{1, 2, 3})}));
        ...
        for (int i = 0; i < totalTests; i++) {
            ...
            if (size % 2 == 0) size++; // 确保是奇数，便于构造多数元素
