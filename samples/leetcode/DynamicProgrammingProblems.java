...
/**
 * LeetCode动态规划相关问题解答 包含0-1背包问题等经典动态规划题目
 */
public class DynamicProgrammingProblems {
  ...
  /**
   * 0-1背包问题 给定n个物品，每个物品有重量weight[i]和价值value[i] 给定背包容量capacity，求能装入背包的物品的最大价值
   * <p>
   * 解题思路：动态规划 dp[i][w] 表示前i个物品在容量为w的背包中能获得的最大价值 状态转移方程： dp[i][w] = max(dp[i-1][w],
  ...
  /**
   * 0-1背包问题（空间优化版本） 时间复杂度：O(n * capacity) 空间复杂度：O(capacity)
   *
   * @param weights  物品重量数组
   * @param values   物品价值数组
   * @param capacity 背包容量
  ...
  /**
   * LeetCode 322. 零钱兑换 给你一个整数数组 coins ，表示不同面额的硬币；以及一个整数 amount ，表示总金额 计算并返回可以凑成总金额所需的最少的硬币个数
   *
   * @param coins  硬币面额数组
   * @param amount 目标金额
  ...
  /**
   * LeetCode 300. 最长递增子序列 给你一个整数数组 nums ，找到其中最长严格递增子序列的长度
   *
   * @param nums 整数数组
   * @return 最长递增子序列长度
   */
  ...
  /**
   * 最长递增子序列（二分优化版本） 时间复杂度：O(n log n)
   *
   * @param nums 整数数组
   * @return 最长递增子序列长度
   */
  ...
  /**
   * LeetCode 1143. 最长公共子序列 给定两个字符串 text1 和 text2，返回这两个字符串的最长公共子序列的长度
   *
   * @param text1 字符串1
   * @param text2 字符串2
   * @return 最长公共子序列长度
  ...
  /**
   * LeetCode 72. 编辑距离 给你两个单词 word1 和 word2， 请返回将 word1 转换成 word2 所使用的最少操作数
  ...
  /**
   * LeetCode 64. 最小路径和 给定一个包含非负整数的 m x n 网格 grid ，请找出一条从左上角到右下角的路径，使得路径上的数字总和为最小
   *
   * @param grid 网格
   * @return 最小路径和
   */
  ...
  /**
   * LeetCode 213. 打家劫舍 II 所有的房屋都围成一圈，这意味着第一个房屋和最后一个房屋是相邻的
   *
   * @param nums 房屋金额数组
   * @return 能偷窃到的最高金额
   */
