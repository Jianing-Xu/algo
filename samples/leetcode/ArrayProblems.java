...
/**
 * LeetCode数组相关问题解答 包含三数之和、求众数、缺失的第一个正数等经典题目
 */
public class ArrayProblems {
  ...
  /**
   * LeetCode 15. 三数之和 给定一个包含 n 个整数的数组 nums，判断 nums 中是否存在三个元素 a，b，c 使得 a + b + c =
   * 0？找出所有满足条件且不重复的三元组
   * <p>
   * 解题思路： 1. 先对数组排序 2. 固定第一个数，用双指针寻找另外两个数 3. 跳过重复元素避免重复解
   * <p>
   * 时间复杂度：O(n²) 空间复杂度：O(1)
   *
   * @param nums 输入数组
  ...
  /**
   * 三数之和的变体：最接近目标值的三数之和 LeetCode 16. 最接近的三数之和
   *
   * @param nums   输入数组
   * @param target 目标值
   * @return 最接近目标值的三数之和
  ...
  /**
   * LeetCode 169. 多数元素 给定一个大小为 n 的数组，找到其中的多数元素 多数元素是指在数组中出现次数大于 ⌊n/2⌋ 的元素
   * <p>
   * 解题思路：Boyer-Moore 投票算法 1. 维护一个候选者和计数器 2. 遇到相同元素计数+1，不同元素计数-1 3. 计数为0时更换候选者
   * <p>
   * 时间复杂度：O(n) 空间复杂度：O(1)
   *
   * @param nums 输入数组
  ...
  /**
   * 多数元素的哈希表解法（用于验证） 时间复杂度：O(n)，空间复杂度：O(n)
   *
   * @param nums 输入数组
   * @return 多数元素
   */
  ...
  /**
   * 多数元素的排序解法 时间复杂度：O(n log n)，空间复杂度：O(1)
   *
   * @param nums 输入数组
   * @return 多数元素
   */
  ...
  /**
   * LeetCode 41. 缺失的第一个正数 给你一个未排序的整数数组 nums，请你找出其中没有出现的最小的正整数
   * <p>
   * 解题思路：原地哈希 1. 将每个正数放到它应该在的位置（nums[i] = i+1） 2. 再次遍历找到第一个不在正确位置的数
   * <p>
   * 时间复杂度：O(n) 空间复杂度：O(1)
   *
   * @param nums 输入数组
  ...
  /**
   * 缺失的第一个正数的标记解法 使用数组本身作为哈希表，用负数标记存在的正数
   *
   * @param nums 输入数组
   * @return 缺失的第一个正数
   */
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "Boyer-Moore算法: " + majority1 + ", 耗时: " + (majorityTime1 / 1000.0) + "μs");
    System.out.println(
        "哈希表算法: " + majorityHash1 + ", 耗时: " + (majorityHashTime1 / 1000.0) + "μs");
    System.out.println(
        "排序算法: " + majoritySort1 + ", 耗时: " + (majoritySortTime1 / 1000.0) + "μs");
    System.out.println(
        "结果一致性: " + (majority1 == majorityHash1 && majorityHash1 == majoritySort1));
