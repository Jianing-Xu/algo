...
public class RecursionProblems {
  ...
  /**
   * LeetCode 70. 爬楼梯 假设你正在爬楼梯。需要 n 阶你才能到达楼顶 每次你可以爬 1 或 2 个台阶。你有多少种不同的方法可以爬到楼顶呢？
   * <p>
   * 解题思路： 这是一个经典的斐波那契数列问题 f(n) = f(n-1) + f(n-2) 到达第n阶的方法数 = 到达第n-1阶的方法数 + 到达第n-2阶的方法数
   * <p>
   * 时间复杂度：O(n) 空间复杂度：O(1)
   *
   * @param n 楼梯阶数
  ...
  /**
   * 爬楼梯的递归解法（会超时，仅用于理解） 时间复杂度：O(2^n) 空间复杂度：O(n)
   *
   * @param n 楼梯阶数
   * @return 爬楼梯的方法数
   */
  ...
  /**
   * 爬楼梯的记忆化递归解法 时间复杂度：O(n) 空间复杂度：O(n)
   *
   * @param n 楼梯阶数
   * @return 爬楼梯的方法数
   */
  ...
  /**
   * 爬楼梯的动态规划解法 时间复杂度：O(n) 空间复杂度：O(n)
   *
   * @param n 楼梯阶数
   * @return 爬楼梯的方法数
   */
  ...
  /**
   * 爬楼梯的矩阵快速幂解法 时间复杂度：O(log n) 空间复杂度：O(1)
   *
   * @param n 楼梯阶数
   * @return 爬楼梯的方法数
   */
  ...
  /**
   * 爬楼梯变体：带成本的爬楼梯 LeetCode 746. 使用最小花费爬楼梯
   *
   * @param cost 每一阶的成本数组
   * @return 到达楼顶的最小成本
   */
   ...
   * @return 执行时间（纳秒）
   */
  public static long testPerformance(int n, String algorithm,
      java.util.function.IntFunction<Integer> function) {
