...
/**
 * LeetCode栈相关问题解答 包含有效括号、最长有效括号、逆波兰表达式求值等经典题目
 */
public class StackProblems {
   ...
   * <p>
   * 解题思路：使用栈 1. 遇到左括号就入栈 2. 遇到右括号就检查栈顶是否为对应的左括号 3. 最后检查栈是否为空
   * <p>
   * 时间复杂度：O(n) 空间复杂度：O(n)
   *
   * @param s 输入字符串
  ...
  /**
   * LeetCode 32. 最长有效的括号 给你一个只包含 '(' 和 ')' 的字符串，找出最长有效（格式正确且连续）括号子串的长度
   * <p>
   * 解题思路：动态规划 dp[i] 表示以下标 i 字符结尾的最长有效括号的长度
   * <p>
   * 时间复杂度：O(n) 空间复杂度：O(n)
   *
   * @param s 输入字符串
   ...
   * 最长有效括号的栈解法
   * <p>
   * 解题思路： 1. 使用栈存储索引 2. 栈底始终保持最后一个没有被匹配的右括号的索引 3. 遇到左括号入栈，遇到右括号出栈并计算长度
   *
   * @param s 输入字符串
   * @return 最长有效括号长度
   */
  ...
  /**
   * LeetCode 150. 逆波兰表达式求值 根据逆波兰表示法，求表达式的值
   * <p>
   * 有效的算符包括 +、-、*、/ 。每个运算对象可以是整数，也可以是另一个逆波兰表达式
   * <p>
   * 解题思路：使用栈 1. 遇到数字就入栈 2. 遇到操作符就弹出两个数字进行运算，结果入栈 3. 最后栈中只剩一个数字，就是结果
   * <p>
   * 时间复杂度：O(n) 空间复杂度：O(n)
   *
  ...
  public static void main(String[] args) {
    ...
    for (String testCase : testCases) {
      ...
      System.out.printf(
          "输入: \"%s\" -> HashMap版本: %s (%.2fμs), 优化版本: %s (%.2fμs), 一致性: %s%n",
