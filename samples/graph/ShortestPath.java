...
/**
 * 最短路径算法实现 包含Dijkstra算法、A*算法、Floyd-Warshall算法等
 */
public class ShortestPath {

  /**
   * Dijkstra算法 - 单源最短路径 适用于非负权重的图 时间复杂度：O((V + E) log V)，使用优先队列优化
   *
   * @param graph  图
   * @param source 源顶点
  ...
  /**
   * Floyd-Warshall算法 - 所有顶点对之间的最短路径 时间复杂度：O(V³)，空间复杂度：O(V²)
   *
   * @param graph 图
   * @return 距离矩阵
   */
  ...
  /**
   * Bellman-Ford算法 - 单源最短路径（支持负权重） 时间复杂度：O(VE)
   *
   * @param graph  图
   * @param source 源顶点
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "大图测试 (顶点数: " + largeVertices + ", 边数: " + largeGraph.getEdgeCount() + ")");
