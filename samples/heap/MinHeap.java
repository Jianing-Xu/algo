...
/**
 * 小顶堆实现 基于数组实现的完全二叉树结构 父节点的值小于等于子节点的值
 */
public class MinHeap<T extends Comparable<T>> {
  ...
  public static void main(String[] args) {
    ...
    while (!heap.isEmpty()) {
      ...
      System.out.println("删除最小值 " + min + ", 剩余: " + heap.toList());
    }
    ...
    // 测试5：性能测试
    System.out.println("\n5. 性能测试:");
    MinHeap<Integer> perfHeap = new MinHeap<>();
    ...
    System.out.println("插入10000个元素并删除5000个最小值耗时: " + (endTime - startTime) + "ms");
    ...
    System.out.println("空堆大小: " + emptyHeap.size());
    System.out.println("空堆是否为空: " + emptyHeap.isEmpty());

    try {
      emptyHeap.peek();
    } catch (IllegalStateException e) {
      System.out.println("空堆peek异常: " + e.getMessage());
    }

    try {
      emptyHeap.extractMin();
    } catch (IllegalStateException e) {
      System.out.println("空堆extractMin异常: " + e.getMessage());
    }

    // 单元素堆
    emptyHeap.insert(42);
    System.out.println("单元素堆: " + emptyHeap.toList());
    System.out.println("堆顶: " + emptyHeap.peek());
    System.out.println("删除堆顶: " + emptyHeap.extractMin());
    System.out.println("删除后是否为空: " + emptyHeap.isEmpty());
    ...
    System.out.println("字符串堆: " + stringHeap.toList());
  ...
  /**
   * 删除并返回堆顶元素（最小值） 将堆底元素移到堆顶，然后向下调整 时间复杂度：O(log n)
   *
   * @return 堆顶元素
  ...
  /**
   * 向上调整（上浮） 将指定位置的元素与其父节点比较，如果小于父节点则交换
   *
  ...
  /**
   * 向下调整（下沉） 将指定位置的元素与其子节点比较，与较小的子节点交换
   *
   * @param index 要调整的元素索引
   */
  ...
  public String toString() {
    return "MinHeap" + toList().toString();
  }
}