...
/**
 * 回溯算法实现集合 包含八皇后问题、0-1背包问题、数独求解、全排列等经典回溯算法
 */
public class BacktrackingAlgorithms {
  ...
  public static void main(String[] args) {
    ...
    for (Knapsack01.Item item : items) {
      System.out.println(
          "  " + item + ", 价值密度: " + String.format("%.2f", item.getValuePerWeight()));
    }
    ...
    for (int testN = 8; testN <= 12; testN++) {
      ...
      System.out.println(
          "N=" + testN + ": " + perfCount + " 个解, 耗时: " + (endTime - startTime) + "ms");
    }
  ...
  /**
   * 八皇后问题解决方案 在8x8棋盘上放置8个皇后，使得它们互不攻击
   */
  ...
  /**
   * 0-1背包问题（回溯法求解） 给定一组物品，每个物品有重量和价值，在限定的背包容量下，选择物品使价值最大
   */
  public static class Knapsack01 {
    ...
    public Knapsack01(Item[] items, int capacity) {
      ...
      Arrays.sort(this.items,
          (a, b) -> Double.compare(b.getValuePerWeight(), a.getValuePerWeight()));
    }
    ...
    /**
     * 计算剩余容量的理论最大价值（用于剪枝） 使用贪心策略按价值重量比选择物品
     *
  ...
  public static class CombinationGenerator {
     ...
     * @param result  结果列表
     */
    private static void backtrack(int start, int n, int k, List<Integer> current,
        List<List<Integer>> result) {
      if (k == 0) {
        result.add(new ArrayList<>(current));
        return;
