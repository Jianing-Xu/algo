...
/**
 * 字符串匹配算法集合 包含朴素匹配、KMP、Boyer-Moore等算法
 */
public class StringMatching {

  /**
   * 朴素字符串匹配算法（暴力匹配） 时间复杂度：O(nm)，其中n是文本长度，m是模式长度 空间复杂度：O(1)
   *
   * @param text    文本字符串
   * @param pattern 模式字符串
  ...
  /**
   * KMP（Knuth-Morris-Pratt）字符串匹配算法 时间复杂度：O(n + m)，空间复杂度：O(m)
   *
   * @param text    文本字符串
   * @param pattern 模式字符串
   * @return 第一次匹配的位置，如果不匹配返回-1
  ...
  /**
   * Boyer-Moore算法的坏字符规则实现 时间复杂度：平均O(n/m)，最坏O(nm)
   *
   * @param text    文本字符串
   * @param pattern 模式字符串
   * @return 第一次匹配的位置，如果不匹配返回-1
  ...
  /**
   * Rabin-Karp算法（滚动哈希） 平均时间复杂度：O(n + m)，最坏时间复杂度：O(nm)
   *
   * @param text    文本字符串
   * @param pattern 模式字符串
   * @return 第一次匹配的位置，如果不匹配返回-1
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "随机测试 " + testCount + " 次，正确率: " + (correctCount * 100.0 / testCount) + "%");
  }
}