...
/**
 * 分治算法实现集合 包含归并排序、快速排序、逆序对计算、最大子数组和等经典分治算法
 */
public class DivideConquerAlgorithms {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "分治算法结果: " + inversionCount + ", 耗时: " + (divideConquerTime / 1000.0) + "μs");
    ...
    for (int size : new int[]{1000, 5000, 10000}) {
      ...
      System.out.println(
          "数组大小: " + size + ", 逆序对数: " + largeInversionCount + ", 耗时: " + largeTime
              + "ms");
    }
    ...
    // 比较三种算法
    startTime = System.nanoTime();
    MaxSubarraySum.SubarrayInfo divideConquerResult = MaxSubarraySum.maxSubarrayDivideConquer(
        subarrayTest);
    ...
    startTime = System.nanoTime();
    MaxSubarraySum.SubarrayInfo bruteForceResult = MaxSubarraySum.maxSubarrayBruteForce(
        subarrayTest);
    ...
    System.out.println(
        "递归版本: " + recursiveResult + ", 耗时: " + (recursiveTime / 1000.0) + "μs");
    System.out.println(
        "迭代版本: " + iterativeResult + ", 耗时: " + (iterativeTime / 1000.0) + "μs");
    ...
    System.out.println(
        "单元素数组最大子数组: " + MaxSubarraySum.maxSubarrayDivideConquer(singleElement));
    ...
    System.out.println(
        "全负数数组最大子数组: " + MaxSubarraySum.maxSubarrayDivideConquer(allNegative));
  ...
  /**
   * 逆序对计算器 使用分治算法计算数组中的逆序对个数 逆序对：对于数组中的两个元素 a[i] 和 a[j]，如果 i < j 但 a[i] > a[j]，则称为一个逆序对
   */
  public static class InversionCounter {

    /**
     * 计算数组中的逆序对个数 时间复杂度：O(n log n)，空间复杂度：O(n)
     *
     * @param array 输入数组
     * @return 逆序对个数
     */
    ...
    public static void printInversions(int[] array) {
      ...
      for (int i = 0; i < array.length - 1; i++) {
        for (int j = i + 1; j < array.length; j++) {
          if (array[i] > array[j]) {
            System.out.println(
                "  (" + array[i] + ", " + array[j] + ") at indices (" + i + ", " + j + ")");
            count++;
          }
        }
      }
  ...
  /**
   * 最大子数组和问题 使用分治算法求解数组中连续子数组的最大和
   */
  public static class MaxSubarraySum {

    /**
     * 使用分治算法求最大子数组和 时间复杂度：O(n log n)
     *
     * @param array 输入数组
     * @return 最大子数组信息
     */
    ...
    /**
     * Kadane算法求最大子数组和（动态规划方法，用于比较） 时间复杂度：O(n)
     *
     * @param array 输入数组
     * @return 最大子数组信息
     */
  ...
  /**
   * 快速幂算法 使用分治思想快速计算 base^exponent % modulo
   */
  public static class FastPower {

    /**
     * 快速幂算法（递归版本） 时间复杂度：O(log n)
     *
     * @param base     底数
     * @param exponent 指数
     * @param modulo   模数
  ...
  /**
   * 大整数乘法（Karatsuba算法） 使用分治算法优化大整数乘法
   */
  public static class BigIntegerMultiplication {

    /**
     * Karatsuba算法实现大整数乘法 时间复杂度：O(n^log2(3)) ≈ O(n^1.585)
     *
     * @param x 第一个数字字符串
     * @param y 第二个数字字符串
     * @return 乘积字符串
